...
class OIDReader:
    ...
    def parse(self):
        label_file = open(self.filepath,"r").readlines()
        for line in label_file:
            line = line.strip('\n')
            line = line.split(' ')
            bbox=[]
            bbox.append('numberplate')
            bbox.append(float(line[-4]))
            bbox.append(float(line[-3]))
            bbox.append(float(line[-2]))
            bbox.append(float(line[-1]))
            self.shapes.append(bbox)
